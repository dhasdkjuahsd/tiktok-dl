...
import zipfile
import shutil
# RapidAPI configuration for TikTok download
RAPIDAPI_TIKTOK_HOST = "tiktok-downloader-download-tiktok-videos-without-watermark.p.rapidapi.com"
RAPIDAPI_TIKTOK_KEY = "f1732e1c0fmsh0fe5a74cc5bb450p140784jsn8f25537eb8db"
RAPIDAPI_TIKTOK_URL = "https://tiktok-downloader-download-tiktok-videos-without-watermark.p.rapidapi.com/rich_response/index"
app = Flask(__name__)
...
class UniversalDownloader:
    ...
    def download_tiktok_content(self, url, path):
        ...
        # Try RapidAPI first, then fall back to yt-dlp if needed
        rapidapi_result = self.download_tiktok_via_rapidapi(url, path)
        if rapidapi_result and rapidapi_result.get('status') == 'success':
    ...
    def download_tiktok_via_rapidapi(self, url, path):
        """Download TikTok by calling RapidAPI"""
        try:
            headers = {
            ...
            response = self.session.get(RAPIDAPI_TIKTOK_URL, headers=headers, params=params, timeout=30)
            data = response.json()

            if response.status_code != 200:
                return {
                    'status': 'error',
                    'message': f'RapidAPI TikTok error: {response.status_code} - {data.get("message", response.text)}'
                }

            # Try common response fields for the video URL
            video_url = None
            if isinstance(data, dict):
                video_url = data.get('video') or data.get('download') or data.get('videoUrl') or data.get('url')
                if not video_url and 'data' in data and isinstance(data['data'], dict):
                    video_url = data['data'].get('video') or data['data'].get('download') or data['data'].get('videoUrl')

            if not video_url:
                return None

            # Save the video from the returned URL
            video_response = self.session.get(video_url, stream=True, timeout=60)
            if video_response.status_code != 200:
                return {
                    'status': 'error',
                    'message': f'Could not download video from RapidAPI URL: {video_response.status_code}'
                }

            filename = os.path.join(path, 'TikTok_RapidAPI_video.mp4')
            ...
            return {
                'status': 'success',
                'message': 'TikTok video downloaded via RapidAPI successfully!',
